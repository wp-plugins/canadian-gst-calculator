...
Copyright 2014 GSTCalculator.ca (info@gstcalculator.ca)
...
include 'cc-canada-gst-calculator-layout.php';


class cc_canada_gst_calculator extends WP_Widget {

	// constructor
	function __construct() {
		$options = array(		
			'name' => __('CC Canadian GST Calculator','cctextdomain'), 
			'description' => __('Canadian GST(Goods and Services Tax) Calculator','cctextdomain')
		);
		parent::__construct('cc_canada_gst_calculator', '', $options);
	}
	...
	function form($instance) {	
        ...
        $defaults = array(
            'title' => __('GST Calculator', 'cctextdomain'),
            'bg_color' => '#ffffff',
            'border_color' => '#ccc',
            'text_color' => '#000000'
        );
	...
	function widget($args, $instance) {
       ...
       //  print_r($args);
		...
		//	 $title = '<a href="http://gstcalculator.ca/" target="_blank" style="text-decoration:none">' . $title . '</a>';		
		load_cc_canada_gst_calc($this->id, $title,  $allow_cc_urls, $bg_color, $border_color, $text_color);
		echo $args['after_widget'];
	}
}

// register widget
add_action('widgets_init', create_function('', 'return register_widget("cc_canada_gst_calculator");'));
...
function cc_gst_admin( $hook_suffix ) {
    // http://make.wordpress.org/core/2012/11/30/new-color-picker-in-wp-3-5/
    wp_enqueue_style( 'wp-color-picker' );
    wp_enqueue_script( 'cc-canada-gst-calculator-admin', plugins_url('cc-canada-gst-calculator-admin.js', __FILE__ ), array( 'wp-color-picker' ), false, true );
}

add_action( 'admin_enqueue_scripts', 'cc_gst_admin' );
